...
public class ContactDealsCreateDialog(IState<bool> isOpen, RefreshToken refreshToken, Guid contactId) : ViewBase
{
    ...
    public override object? Build()
    {
        ...
        return deal
            ...
            .Builder(e => e.OwnerId, e => e.ToAsyncSelectInput(QueryOwners(factory), LookupOwner(factory), placeholder: "Select Owner"))
    ...
    private static AsyncSelectQueryDelegate<Guid?> QueryOwners(DataContextFactory factory)
    {
        return async query =>
        {
            await using var db = factory.CreateDbContext();
    ...
    private static AsyncSelectLookupDelegate<Guid?> LookupOwner(DataContextFactory factory)
    {
        return async id =>
        {
            if (id == null) return null;
